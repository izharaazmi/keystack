...
// Get all teams for a specific user (admin only)
router.get('/:id/teams', adminAuth, async (req, res) => {
	try {
		const {id} = req.params;
...
router.delete('/:id/teams/:teamId', adminAuth, async (req, res) => {
	try {
		...
		const userGroup = await UserGroup.findOne({
			where: {userId: id, groupId: teamId}
		});
		...
		res.json({message: 'Successfully removed user from team'});
	} catch (error) {
		res.status(500).json({message: 'Server error'});
	}
});
